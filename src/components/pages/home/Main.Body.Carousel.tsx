...
const MainBodyCarousel = (): JSX.Element => {
  ...
  }, [handleNextCarouselClick, windowWidth]);
          ...
          (service, i) => (
            <RevealOnScroll
              ...
              className={`${S.serviceCardContainer}  ${
                activeServiceIndex === i ? S.serviceActive : ''
              } mt-4 d-flex justify-content-center justify-content-md-start`}
              allowOverflow
              delay={0.25}>
              <Box
                className={`${S.serviceCard}`}
                data-anim={i % 2 === 0 ? 'fadeInLeft' : 'fadeInRight'}>
                <Box as="h3" className="my-0 h5 d-inline-flex align-items-center">
                  <SVGIcon name={service.icon} size="medium" className="me-3" /> {service.name}
                </Box>
                <Box as="p" className="mt-2 mb-0">
                  {service.description}
                </Box>
              </Box>
            </RevealOnScroll>

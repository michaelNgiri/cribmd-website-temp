...
export class GetImage {
  ...
  private static blackFridayUrl = GetImage.baseUrl + 'black-friday/';
  ...
  static blackFriday(name: string, size?: '1x' | '2x') {
    return `${this.blackFridayUrl}${name}__${parseInt(String(size || 1)) * 450}x.webp`;
  }
  ...
  static advert(name: string, size?: '1x' | '2x') {
    return `${this.baseUrl}_adverts/${name}__${parseInt(String(size || 1)) * 200}x.webp`;
  }
}
